...
void printing_configuration(database_type *database) {
    ...
    database->file_information->print_conf = ask_for_answer() << 3 | database->file_information->print_conf;
    ...
    database->file_information->print_conf = ask_for_answer() << 5 | database->file_information->print_conf;
    printf("...the last time of editing? (y/n)\n");
    database->file_information->print_conf = ask_for_answer() << 6 | database->file_information->print_conf;
}//    printf("...the refreshing? (y/n)\n");
//    database->file_information->print_conf = ask_for_answer()<<7 | database->file_information->print_conf;
...
void print_article(struct article_type article, unsigned int p_conf) {
    ...
    if (p_conf & 1 << 5) {
        printf("%4d.%02d.%02d %2d:%2d:%2d   ", time_created.tm_mday, time_created.tm_mon + 1,
               time_created.tm_year + 1900,
...
} //could be solved with binary search
...
int binary_search_article_in_range(struct database_type *database, char *searched_article, int left_boundary,
                                   int right_boundary) {
    if (database->file_information->sorting_mode != name_a_z) {
        return -1;
    }
